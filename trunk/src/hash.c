...
#define _aspis_hash four_char_hash
#else
#ifdef WANT_SDBM_HASH
static unsigned sdbm_hash(const char *str);
#define _aspis_hash sdbm_hash
#else
#ifdef WANT_DJB2_HASH
static unsigned djb2_hash(const char *str);
#define _aspis_hash djb2_hash
#else
static unsigned fnv1a_hash(const char *str);
#define _aspis_hash fnv1a_hash
#endif
#endif
#endif

static unsigned aspis_hash(const char *str)
{
    if (str == NULL || str[0] == '\0') {
        log_error_time();
        fprintf(stderr,
                "Attempt to hash NULL or empty string! [aspis_hash]!\n");
        return 0;
    }
    return _aspis_hash(str);
}
...
{
    return aspis_hash(extension) % MIME_HASHTABLE_SIZE;
}
...
static unsigned get_homedir_hash_value(const char *name)
{
    return aspis_hash(name) % PASSWD_HASHTABLE_SIZE;
}
