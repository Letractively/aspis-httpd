...
globals.h:
#ifdef INET6
	char remote_ip_addr[ASPIS_NI_MAXHOST];
#else
	char remote_ip_addr[20];        after inet_ntoa
#endif

    None of this code interacts with the rest of Aspis except through
...
#error The following two lines are broken
        memmove(dest, dest + 7, ASPIS_NI_MAXHOST);
        dest[ASPIS_NI_MAXHOST] = '\0';
    }
