...
static void create_argv(request * req, char **aargv)
{
     ...
     * into words, according to the following rules:
     ...
     Since aspis builds the argument list and does not call /bin/sh,
     (aspis uses execve for CGI)
     */
...
int init_cgi(request * req)
{
    ...
    case -1:
         ...
         * aspis_perror) would work for NPH and CGI, but not for GUNZIP.  
         * Fix that. 
         */
        aspis_perror(req, "fork failed");
        if (use_pipes) {
            close(pipes[0]);
            close(pipes[1]);
        }
    ...
    case 0:
        ...
        if (cgi_log_fd) {
			...
			dup2(cgi_log_fd, open("/dev/null", 0));
		}
        ...
        if (req->cgi_type == PHP) {
			...
			aargv[0] = cpt;
			execve(php_handler, aargv, req->cgi_env);
        } else if (req->cgi_type) {
            char *aargv[CGI_ARGC_MAX + 1];
            create_argv(req, aargv);
    ...
    default:
        ...
        if (req->cgi_type & 0x02) { /* CGI (10b) or PHP (11b) */
