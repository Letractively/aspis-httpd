...
int unescape_uri(unsigned char *uri, char **query_string)
{
    char c, d;
    char *uri_old;
...
{
    static char aspis_tempfile[MAX_PATH_LENGTH + 1];
    int fd;

    snprintf(aspis_tempfile, MAX_PATH_LENGTH, "%s/aspis-temp.XXXXXX", tempdir);

    /* open temp file */
    fd = mkstemp(aspis_tempfile);
    if (fd == -1) {
        log_error_time();
        perror("mkstemp");
        return 0;
    ...
    if (storage != NULL) {
        unsigned int len = strlen(aspis_tempfile);

        if (len < size) {
            memcpy(storage, aspis_tempfile, len + 1);
        } else {
            close(fd);
            fd = 0;
            log_error_time();
