...
static void free_request(request * req)
{
    ...
    if (req->data_fd) {
        close(req->data_fd);
        ASPIS_FD_CLR(req, req->data_fd, ASPIS_READ);
    }

    if (req->post_data_fd) {
        close(req->post_data_fd);
        ASPIS_FD_CLR(req, req->post_data_fd, ASPIS_WRITE);
    }
    ...
    if (req->status < TIMED_OUT && (req->keepalive == KA_ACTIVE) &&
        ...
        ASPIS_FD_SET(req, req->fd, ASPIS_READ);
        ASPIS_FD_CLR(req, req->fd, ASPIS_WRITE);
        return;
    }
    ...
    /*
       ...
       Aspis will (for now) hack around this stupid bug in Netscape
    ...
    close(req->fd);
    ASPIS_FD_CLR(req, req->fd, ASPIS_READ);
    ASPIS_FD_CLR(req, req->fd, ASPIS_WRITE);
    total_connections--;
...
int process_header_end(request * req)
{
    ...
    if (req->method == M_POST) {
        ...
        if (fcntl(req->post_data_fd, F_SETFD, 1) == -1) {
            aspis_perror(req, "unable to set close-on-exec for req->post_data_fd!");
            close(req->post_data_fd);
            req->post_data_fd = 0;
            return 0;
        }
