...
void print_ka_phrase(request * req)
{
    if (req->kacount > 0 &&
        ...
        /* For HTTP 1.0 refer to Roy Fielding (1995)
         * http://ftp.ics.uci.edu/pub/ietf/http/hypermail/1995q4/0063.html
         */
...
void write_error_page_body(request * req, int n)
{    /* we assume that v_errorpages[n] exists and is readable 
		since we checked both conditions in config.c */
	int b_read, data_fd = open(v_errorpages[n], O_RDONLY);
	char buf[2560];

	b_read = read(data_fd, buf, 2560);
	req_write(req, buf);
	close(data_fd);
 ...
 * The rest of Aspis only enters through the routines below.
 */
...
void send_r_bad_request(request * req)
{
    ...
    if (req->method != M_HEAD) {
        if (v_errorpages[ERRORPAGE_400] != NULL) {
			write_error_page_body(req, ERRORPAGE_400);
		} else {
			req_write(req,
...
void send_r_unauthorized(request * req, const char *realm_name)
{
    ...
    if (req->method != M_HEAD) {
        if (v_errorpages[ERRORPAGE_401] != NULL) {
			write_error_page_body(req, ERRORPAGE_401);
		} else {
			req_write(req,
...
void send_r_forbidden(request * req)
{
    ...
    if (req->method != M_HEAD) {
        if (v_errorpages[ERRORPAGE_403] != NULL) {
			write_error_page_body(req, ERRORPAGE_403);
		} else {
...
void send_r_not_found(request * req)
{
    ...
    if (req->method != M_HEAD) {
		if (v_errorpages[ERRORPAGE_404] != NULL) {
			write_error_page_body(req, ERRORPAGE_404);
		} else {
...
void send_r_length_required(request * req)
{
    ...
    if (req->method != M_HEAD) {
        if (v_errorpages[ERRORPAGE_411] != NULL) {
			write_error_page_body(req, ERRORPAGE_411);
		} else {
			req_write(req,
...
void send_r_precondition_failed(request * req)
{
    ...
    if (req->method != M_HEAD) {
        if (v_errorpages[ERRORPAGE_412] != NULL) {
			write_error_page_body(req, ERRORPAGE_412);
		} else { 
			req_write(req,
...
void send_r_request_uri_too_long(request * req)
{
    ...
    if (req->method != M_HEAD) {
        if (v_errorpages[ERRORPAGE_414] != NULL) {
			write_error_page_body(req, ERRORPAGE_414);
		} else { 
			req_write(req, body);
		}
	}
...
void send_r_invalid_range(request * req)
{
    ...
    if (req->method != M_HEAD) {
        if (v_errorpages[ERRORPAGE_416] != NULL) {
			write_error_page_body(req, ERRORPAGE_416);
		} else {
			req_write(req, body);
		}
    }
