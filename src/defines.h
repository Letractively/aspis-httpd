...
/***** Change this via the CGIPath configuration value in aspis.conf *****/
#define DEFAULT_PATH     "/bin:/usr/bin:/usr/local/bin"

/***** Change this via the DefaultVHost configuration directive in aspis.conf *****/
#define DEFAULT_VHOST "default"
#define DEFAULT_CONFIG_FILE "aspis.conf" /* locate me in the server root */

/***** Change this via the SinglePostLimit configuration value in aspis.conf *****/
#define SINGLE_POST_LIMIT_DEFAULT               1024 * 1024 /* 1 MB */

/***** You may change this safely, maybe *****/
#define MAX_INDEX_NAME_LENGTH		16
#define NUM_OF_ERRORPAGES			8
...
/*************** POLL / SELECT MACROS *******************/
#ifdef HAVE_POLL
#define ASPIS_READ (POLLIN|POLLPRI|POLLHUP)
#define ASPIS_WRITE (POLLOUT|POLLHUP)
#define ASPIS_FD_SET(req, thefd,where) { struct pollfd *my_pfd = &pfds[pfd_len]; req->pollfd_id = pfd_len++; my_pfd->fd = thefd; my_pfd->events = where; }
#define ASPIS_FD_CLR(req, fd, where) /* this doesn't do anything? */
#else                           /* SELECT */
#define ASPIS_READ (&block_read_fdset)
#define ASPIS_WRITE (&block_write_fdset)
#define ASPIS_FD_SET(req, fd, where) { FD_SET(fd, where); if (fd > max_fd) max_fd = fd; }
#define ASPIS_FD_CLR(req, fd, where) { FD_CLR(fd, where); }
#endif
