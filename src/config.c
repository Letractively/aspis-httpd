...
char a_directory_index[4][MAX_INDEX_NAME_LENGTH] = {"","","",""};
char cgi_directory_index[4][MAX_INDEX_NAME_LENGTH] = {"","","",""};
char php_directory_index[4][MAX_INDEX_NAME_LENGTH] = {"","","",""};
char stc_directory_index[4][MAX_INDEX_NAME_LENGTH] = {"","","",""};
char *default_type;
...
char *v_errorpages[NUM_OF_ERRORPAGES];
...
#ifdef USE_SETRLIMIT
extern int cgi_rlimit_cpu;      /* aspis.c */
extern int cgi_rlimit_data;     /* aspis.c */
extern int cgi_nice;            /* aspis.c */
#endif
...
struct ccommand clist[] = {
    ...
    {"ErrorDocument", S2A, c_set_errorpage, NULL},
};
...
static void c_set_errorpage (char *v1, char *v2, void *t)
{
	...
	/* absolute path */
	if (v2[0] == '/') {
		memcpy(path, v2, strlen(v2) + 1);
	} else { /* relative path */
		int len = strlen(server_root);
		
		memcpy(path, server_root, len);
		memcpy(path + len + 1, v2, strlen(v2) + 1);
	}
	
	if (access(path, R_OK) < 0) {
		fputs("ErrorDocument: file does not exist or cannot be read.\n", stderr);
		/* it has some problem with chroot */
		/* return; */
	}
	
	switch(i) {
		case 400:
			v_errorpages[ERRORPAGE_400] = strdup(v2);
			break;
		case 401:
			v_errorpages[ERRORPAGE_401] = strdup(v2);
			break;
		case 403:
			v_errorpages[ERRORPAGE_403] = strdup(v2);
			break;
		case 404:
			v_errorpages[ERRORPAGE_404] = strdup(v2);
			break;
		case 411:
			v_errorpages[ERRORPAGE_411] = strdup(v2);
			break;
		case 412:
			v_errorpages[ERRORPAGE_412] = strdup(v2);
			break;
		case 414:
			v_errorpages[ERRORPAGE_414] = strdup(v2);
			break;
		case 416:
			v_errorpages[ERRORPAGE_416] = strdup(v2);
			break;
	}
...
/* split directory_index */

int get_indices (void) 
{
	unsigned short int c = 0, i = 0,
	ls = strlen(directory_index);
	
	for (;i < 4; i++) {
		unsigned short int start = c;
		for (; c <= ls; ++c) {
			if (directory_index[c] == ' ' || directory_index[c] == '\0') {
				if ((c - start) > MAX_INDEX_NAME_LENGTH)
					return 1; /* index file name is too long */
				memcpy(a_directory_index[i], directory_index + start,
				c - start);
				a_directory_index[i][c - start] = '\0';
				++c;
				break;
			}
		}
...
void read_config_files(void)
{
    ...
    if (!config) {
        fputs("Could not open aspis.conf for reading.\n", stderr);
        exit(EXIT_FAILURE);
    }
    parse(config);
    fclose(config);
    ...
    if (!server_name) {
		/* host name cannot exceed the 255 bytes limit */
        char temp_name[255];
		struct addrinfo hints, *res;
		
        if (gethostname(temp_name, 255) == -1) {
            perror("gethostname");
            exit(EXIT_FAILURE);
        }
        ...
        hints.ai_flags = AI_CANONNAME; /* this is it */
		
		if (getaddrinfo(temp_name, NULL, &hints, &res) != 0) {
			perror("getaddrinfo");
            exit(EXIT_FAILURE);
		}
        
        server_name = strdup(res->ai_canonname);
        if (server_name == NULL) {
            perror("strdup");
            exit(EXIT_FAILURE);
        }
    ...
    if (default_type == NULL) {
     ...
     if (php_handler == NULL) { /* set default value */
		php_handler = strdup(DEFAULT_PHP_HANDLER);
	}
	
	if (directory_index != NULL) {
		if (get_indices())
		  DIE("Malformed directory index list!");
	}

	/* simple hack to determine if we use php, I had not better ideas... */
	if (strcmp(PHP_MIME_TYPE, get_mime_type("index.php")) == 0) {
		php_usage = 1;
	}

	/* Let's catalogue our index files by type: static, cgi, php */
	{
		unsigned int i = 0, j = 0, k = 0, q;
		for (q = 0; q < 4; q++) {
			if (a_directory_index[q][0] == '\0')
				break;
			if (strcmp(CGI_MIME_TYPE, get_mime_type(a_directory_index[q])) == 0) {
				memcpy(cgi_directory_index[i], a_directory_index[q], MAX_INDEX_NAME_LENGTH);
				i++;
			} else if (strcmp(PHP_MIME_TYPE, get_mime_type(a_directory_index[q])) == 0) {
				memcpy(php_directory_index[j], a_directory_index[q], MAX_INDEX_NAME_LENGTH);
				j++;
			} else {
				memcpy(stc_directory_index[k], a_directory_index[q], MAX_INDEX_NAME_LENGTH);
				k++;
			}
		}
	}
