...
int translate_uri(request * req)
{
    ...
    if (current) {
        ...
        if (current->type == REDIRECT) { /* Redirect */
            ...
            if (!req->pathname) {
                aspis_perror(req, "unable to strdup buffer onto req->pathname");
                return 0;
            }
            return 1;
        }
    ...
    if (user_dir && req->request_uri[1] == '~') {
        ...
        if (l1 + 1 + l2 + 1 + l3 + 1 + l4 + uri_len + 1 > sizeof(buffer)) {
            log_error_doc(req);
            fputs("uri too long!\n", stderr);
            send_r_bad_request(req);
            return 0;
        ...
        if (l1 + 1 + uri_len + l3 + 1 > sizeof(buffer)) {
            log_error_doc(req);
            fputs("uri too long!\n", stderr);
            send_r_bad_request(req);
            return 0;
        ...
        if (virtualhost) {
            ...
            memcpy(buffer + l1 + 1 + l3, req->request_uri, uri_len + 1);
        } else
            memcpy(buffer + l1, req->request_uri, uri_len + 1);
    } else {
    ...
    req->pathname = strdup(buffer);
    if (!req->pathname) {
        aspis_perror(req, "Could not strdup buffer for req->pathname!");
        return 0;
    }
    ...
    if (strcmp(CGI_MIME_TYPE, get_mime_type(req->pathname)) == 0) { /* cgi */
        ...
        if (req->http_version == HTTP09)
            ...
            req->cgi_type = CGI;
        return 1;
    } else if (strcmp(PHP_MIME_TYPE, get_mime_type(req->pathname)) == 0) { /* php-cgi */
		req->script_name = strdup(req->request_uri);
        if (!req->script_name) {
            aspis_perror(req, "Could not strdup req->request_uri for req->script_name.");
            return 0;
		...
		if (i == 0) {
			...
			if (S_ISDIR (st_check.st_mode)) {
				if ((lon + MAX_INDEX_NAME_LENGTH) > MAX_PATH_LENGTH) {
					errno = ENOMEM;
					aspis_perror(req, "pathname + index is too large");
					return -1;
				}
				if (st[lon - 1] != '/') {
					char nurl[uri_len + 1];
					memcpy(nurl, req->request_uri, uri_len);
					nurl[uri_len] = '/';
					nurl[uri_len + 1] = '\0';
					send_r_moved_temp(req, nurl, "");
					/* it should never get down here, however... */
					return -1;
				}
				...
				if (i) i = 0;
				while (cgi_directory_index[i][0] != '\0') {
					memcpy(st + lon, cgi_directory_index[i], MAX_INDEX_NAME_LENGTH + 1);
					if (access(st, F_OK) == 0) {
						unsigned int lan = strlen(cgi_directory_index[i]) ;
						memcpy(st, req->pathname, lpn);
				...
				while (stc_directory_index[i][0] != '\0') {
					...
					if (access(st, F_OK) == 0) {
						unsigned int lan = strlen(stc_directory_index[i]) ;
						req->pathname = strdup(st);
						memcpy(req->request_uri, stc_directory_index[i], lan + 1);
						goto END;
					}
					i++;
				}
			}
			/* it exists but it is not a script */
				goto END;
		} else {
			/* perhaps it's an aliased URL containing a script 
			 * e.g. myhost.com/mystuff/stuff.php/path
			 */
			char *c = 0;
			char ss[MAX_PATH_LENGTH];
			/* php/cgi index */
			i = 0;
			memcpy(ss, req->request_uri, uri_len);
			c = strrchr(ss, (int) '/');
			while (c != NULL ) {
				i = c - ss;
				ss[i] = '\0';
...
static int init_script_alias(request * req, alias * current1, unsigned int uri_len)
{
...
#endif
    /* SGC Do php scripts from cgi-bin directory need to be executed
     * as php? 
     */
    ...
    req->script_name = strdup(req->request_uri);
    if (!req->script_name) {
        aspis_perror(req, "unable to strdup req->request_uri for req->script_name");
        return 0;
    }
    ...
    /* we have path_info if c == '/'... still have to check for query */
        ...
        if (!req->path_info) {
            aspis_perror(req, "unable to strdup pathname + index for req->path_info");
            return 0;
        }
            ...
            if (!strncmp(req->path_info, current->fakename,
                ...
                */
                ...
                if (!req->path_translated) {
                    aspis_perror(req, "unable to strdup buffer for req->path_translated");
                    return 0;
                }
            }
            current = current->next;
        ...
        if (!req->path_translated && user_dir && req->path_info[1] == '~') {
            ...
            }
            {
                ...
                if (req->path_translated == NULL) {
                    aspis_perror(req, "unable to malloc memory for req->path_translated");
                    return 0;
                }
    ...
    if (!req->pathname) {
        aspis_perror(req, "unable to strdup pathname for req->pathname");
        return 0;
    }
