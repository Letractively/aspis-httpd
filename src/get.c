...
int init_get(request * req)
{
    ...
    if (data_fd == -1 && errno == ENOENT) {
        ...
        if (data_fd != -1) {
            ...
            if (!req->pathname) {
                aspis_perror(req, "strdup req->pathname for gzipped filename " __FILE__ ":" STR(__LINE__));
                return 0;
            }
    ...
    if (S_ISDIR(statbuf.st_mode)) { /* directory */
        ...
        if (req->pathname[strlen(req->pathname) - 1] != '/') {
            ...
            if (!port && server_port != 80) {
                ...
                if (port == NULL) {
                    errno = ENOMEM;
                    aspis_perror(req, "Unable to perform simple_itoa conversion on server port!");
                    return 0;
                }
                l3 = strlen(port);
            }
            ...
            if (server_port != 80) {
                if (l4 + l2 + 1 + l3 + len + 1 > sizeof(buffer)) {
                    errno = ENOMEM;
                    aspis_perror(req, "buffer not large enough for directory redirect");
                    return 0;
                }
...
static int get_cachedir_file(request * req, struct stat *statbuf)
{
    ...
    if (data_fd != -1) {        /* index cache */
        ...
        if (statbuf->st_mtime > real_dir_mtime) {
            ...
            strcpy(req->request_uri, DEFAULT_DIR_INDEX); /* for mimetype */
            return data_fd;
        }
        close(data_fd);
    ...
    aspis_perror(req, "re-opening dircache");
    return -1;                  /* Nothing worked. */
...
static int index_directory(request * req, char *dest_filename)
{
    ...
    if (fdstream == NULL) {
        aspis_perror(req, "dircache fopen");
    ...
    while ((dirbuf = readdir(request_dir))) {
        ...
        escname = html_escape_string(dirbuf->d_name, NULL);
        if (escname != NULL) {
#ifndef SOLARIS
