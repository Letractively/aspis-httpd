...
void loop(int server_s)
{
    ...
    while (1) {
        ...
        if (sigterm_flag) {
            ...
            if (sigterm_flag == 1) {
                sigterm_stage1_run();
                ASPIS_FD_CLR(req, server_s, ASPIS_READ);
                close(server_s);
            ...
            if (total_connections > max_connections) {
                ...
                ASPIS_FD_SET(req, server_s, ASPIS_READ); /* server always set */
            }
        }
        ...
        if (max_fd) {
             ...
             * and others.
             */
            if (!sigterm_flag && FD_ISSET(server_s, ASPIS_READ)) {
                pending_requests = 1;
            }
...
static void fdset_update(void)
{
    ...
    for (current = request_block; current; current = next) {
        ...
        if (current->buffer_end && /* there is data to write */
            current->status < DONE) {
            if (FD_ISSET(current->fd, ASPIS_WRITE))
                ready_request(current);
            else {
                ASPIS_FD_SET(current, current->fd, ASPIS_WRITE);
            }
        } else {
            ...
            case IOSHUFFLE:
                ...
                if (current->buffer_end - current->buffer_start == 0) {
                    if (FD_ISSET(current->data_fd, ASPIS_READ))
                        ready_request(current);
                    break;
                }
#endif
            ...
            case PIPE_WRITE:
                ...
                break;
            case BODY_WRITE:
                if (FD_ISSET(current->post_data_fd, ASPIS_WRITE))
                    ready_request(current);
                else {
                    ASPIS_FD_SET(current, current->post_data_fd,
                               ASPIS_WRITE);
                }
                break;
            case PIPE_READ:
                if (FD_ISSET(current->data_fd, ASPIS_READ))
                    ...
                    ASPIS_FD_SET(current, current->data_fd,
                               ASPIS_READ);
                }
                break;
            case DONE:
            ...
            case DEAD:
                ...
                break;
            default:
                if (FD_ISSET(current->fd, ASPIS_READ))
                    ready_request(current);
                else {
                    ASPIS_FD_SET(current, current->fd, ASPIS_READ);
                }
                break;
            }
        }
